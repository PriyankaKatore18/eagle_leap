...
export function HomeHeroCarousel() {
  return (
    <section className="relative isolate overflow-hidden pt-[4.5rem] lg:pt-20">
      ...
      <div className="relative grid min-h-[calc(100vh-4.5rem)] w-full items-center gap-12 px-6 py-12 sm:px-8 md:py-14 lg:min-h-[42rem] lg:grid-cols-[1.02fr_0.98fr] lg:px-16 lg:py-16 xl:px-24 2xl:px-28">
        <div className="max-w-3xl animate-slide-in-left">
          <div className="inline-flex items-center gap-2 rounded-full border border-accent/35 bg-white/10 px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.26em] text-accent backdrop-blur-sm sm:text-xs">
            <span className="h-2.5 w-2.5 rounded-full bg-accent shadow-glow" />
            A Leading National Book Publishing House in India
          </div>

          <h1 className="mt-7 max-w-[12ch] text-[2.9rem] font-extrabold leading-[0.96] tracking-[-0.05em] text-white [text-shadow:0_10px_40px_rgba(0,0,0,0.42)] sm:max-w-none sm:text-[4rem] lg:text-[4.65rem] xl:text-[5.2rem]">
            <span className="block">Publish Your Book</span>
            <span className="block text-gradient-accent">with Eagle Leap Publication</span>
          </h1>

          <p className="mt-6 max-w-2xl text-base font-medium leading-relaxed text-white drop-shadow-[0_8px_18px_rgba(0,0,0,0.45)] sm:text-lg lg:text-xl">
          ...
          <div className="mt-8 flex flex-col gap-4 sm:flex-row">
            <Button
              asChild
              size="lg"
              className="h-14 rounded-2xl bg-accent px-8 text-base font-bold text-accent-foreground shadow-[0_20px_50px_-20px_rgba(255,124,26,0.82)] hover:bg-accent/90 sm:h-16 sm:text-lg"
            >
            ...
            <Button
              ...
              size="lg"
              variant="outline"
              className="h-14 rounded-2xl border-white/30 bg-white/10 px-8 text-base font-bold text-white backdrop-blur-sm hover:bg-white/15 hover:text-white sm:h-16 sm:text-lg"
            >
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            {["Books", "Edited Books", "Journals", "ISBN", "Printing", "Distribution"].map((item) => (
              <span
                key={item}
                className="rounded-full border border-white/12 bg-white/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-white/90 backdrop-blur-sm sm:text-[0.76rem]"
              >
                {item}
              </span>
            ))}
