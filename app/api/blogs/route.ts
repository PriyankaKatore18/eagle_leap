...
export async function GET(request: Request) {
  ...
  const blogs = content.blogs.map((blog) => ensureBlogRecord(blog));
  return json(blogs, request);
}
...
export async function POST(request: Request) {
  ...
  const record = ensureBlogRecord({
    id: String(body.id ?? createBlogRecordId()),
  ...
  const blogs = [record, ...current.blogs.filter((blog) => blog.id !== record.id)];
  const saved = await saveCmsContent({
    ...current,
    blogs,
  });

  return json(saved.blogs.find((blog) => blog.id === record.id) ?? record, request, { status: 201 });
}
